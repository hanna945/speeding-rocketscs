...
//
// meta-api-brands(品牌清單)、meta-api-cpa-thresholds(CPA門檻設定)這兩個是「全部品牌共用一份」的 key,
// 品牌範圍受限的身份(brands 是陣列,不是 "*")存取時:
//   - GET:只回傳自己權限內的品牌,不會看到其他部門的品牌名稱/ID。
...
const SHARED_LIST_KEYS = new Set(["meta-api-brands", "meta-api-cpa-thresholds"]);
...
function filterSharedValue(key, rawValue, credential) {
  ...
  if (key === "meta-api-brands" && Array.isArray(parsed)) {
    return JSON.stringify(parsed.filter((b) => b && credential.brands.includes(b.id)));
  }
  if (key === "meta-api-cpa-thresholds" && parsed && typeof parsed === "object") {
    const filtered = {};
...
async function mergeScopedWrite(env, key, incomingRaw, credential) {
  ...
  if (key === "meta-api-cpa-thresholds") {
...
export async function onRequestDelete({ params, env, data }) {
  ...
  if (SHARED_LIST_KEYS.has(key) && data.credential.brands !== "*") {
    ...
    const emptyValue = key === "meta-api-brands" ? "[]" : "{}";
